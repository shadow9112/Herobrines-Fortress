...
public class MCreatorFirethrowingdagger extends Elementsherobrines_fortress.ModElement {
	@GameRegistry.ObjectHolder("herobrines_fortress:firethrowingdagger")
	...
	public void initElements() {
		...
		elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityArrowCustom.class)
				.id(new ResourceLocation("herobrines_fortress", "entitybulletfirethrowingdagger"), ENTITYID).name("entitybulletfirethrowingdagger")
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("herobrines_fortress:firethrowingdagger", "inventory"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		RenderingRegistry.registerEntityRenderingHandler(EntityArrowCustom.class, renderManager -> {
			return new RenderSnowball(renderManager, new ItemStack(MCreatorFiresword.block, (int) (1)).getItem(), Minecraft.getMinecraft()
					.getRenderItem());
		});
	}
	...
	public static class RangedItem extends Item {
		public RangedItem() {
			...
			setMaxDamage(1);
			setFull3D();
			setUnlocalizedName("firethrowingdagger");
			...
			if (!world.isRemote && entityLivingBase instanceof EntityPlayerMP) {
				...
				for (int i = 0; i < entity.inventory.mainInventory.size(); i++) {
					...
					if (stack != null && stack.getItem() == new ItemStack(MCreatorFirethrowingdagger.block, (int) (1)).getItem()
							&& stack.getMetadata() == new ItemStack(MCreatorFirethrowingdagger.block, (int) (1)).getMetadata()) {
						slotID = i;
						break;
					}
				}
				if (entity.capabilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel(Enchantments.INFINITY, itemstack) > 0 || slotID != -1) {
					...
					world.playSound((EntityPlayer) null, (double) x, (double) y, (double) z,
							(net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY.getObject(new ResourceLocation(
									("block.fire.ambient"))), SoundCategory.NEUTRAL, 1, 1f / (itemRand.nextFloat() * 0.5f + 1f) + (power / 2));
					if (entity.capabilities.isCreativeMode) {
						entityarrow.pickupStatus = EntityArrow.PickupStatus.CREATIVE_ONLY;
					} else {
						if (new ItemStack(MCreatorFirethrowingdagger.block, (int) (1)).isItemStackDamageable()) {
							...
							entity.inventory.clearMatchingItems(new ItemStack(MCreatorFirethrowingdagger.block, (int) (1)).getItem(), -1, 1, null);
						}
					}
					if (!world.isRemote)
